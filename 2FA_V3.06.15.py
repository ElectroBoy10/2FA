...
═══════════════════════════════════════════════════
  ...
  Quick start:
      ...
      # Verify a code
      ok = twofa.verify_totp(result.secret, "123456")
      print(ok)              # → True / False
...
═══════════════════════════════════════════════════
  ...
  V3.06.10 — 3rd-party API + headless mode
    ...
    + verify_totp() / verify_hotp() — public verification functions
...
def verify_totp(
    ...
    Args:
        ...
        code:         The OTP code from the authenticator app.
        valid_window: How many 30s windows either side to allow for clock
                      drift (default: 1 = ±30 seconds, standard tolerance).
    ...
    Raises:
        ...
        TwoFAError:         if pyotp raises unexpectedly.
    """
    if not _PYOTP_AVAILABLE:
        raise TwoFAError("pyotp is required.  Install with:  pip install pyotp")
    validate_secret(secret)
    try:
        ...
        raise TwoFAError(f"TOTP verification error: {e}") from e
...
def verify_hotp(
    ...
    """
    Verify an HOTP code against a known secret and counter value.

    Args:
        secret:  The base32 secret key.
        code:    The OTP code from the authenticator app.
        counter: The expected counter value for this code.
    ...
    try:
        ...
        raise TwoFAError(f"HOTP verification error: {e}") from e
...
def _verify_otp_interactive() -> None:
    """
    Interactive OTP verification screen.

    Prompts for a secret and live code, then calls the public verify_totp /
    verify_hotp API functions — no verification logic lives here.
    """
    _clear_screen()
    _banner("OTP Verification")
    print(
    ...
    otp_type = (
        input(
            f"{Fore.YELLOW}Type \u2014 [T]OTP or [H]OTP (default T): "
            f"{Style.RESET_ALL}"
        ).strip().upper() or "T"
    )
    ...
    if not code:
        _print_error("No code entered.")
        _pause()
        return

    try:
        if otp_type == "H":
            counter  = _prompt_int("Counter value", default=0, min_val=0)
            is_valid = verify_hotp(secret, code, counter)
        else:
            is_valid = verify_totp(secret, code)

        if is_valid:
            _print_success("Code is VALID.")
        else:
    ...
    except (InvalidSecretError, TwoFAError) as e:
        _print_error(str(e))
...
{Fore.WHITE}  Import this file via importlib and call the public API directly.
  ...
  Public functions: create_totp, create_hotp, render_qr,
                    verify_totp, verify_hotp, generate_secret
